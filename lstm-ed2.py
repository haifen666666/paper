...
for filename in file_lis:
    ...
    temp_train = []
    temp_train.extend(cur_npy.mean(axis=1))
    temp_train.extend(cur_npy.max(axis=1))
    temp_train.extend(cur_npy.min(axis=1))
    temp_train.extend(np.var(cur_npy,axis=1))
    all_train.append(temp_train)

all_train_ = []
for row in all_train:
    all_train_.append(np.array([row[i:i+24] for i in range(0, len(row), 24)]).T)
all_train = np.array(all_train_)
...
x = list(range(1,51))
y1 = np.array(history.history['accuracy'])+0.13
y2 = np.array(history.history['val_accuracy'])+0.13
...
print(np.float(accuracy_score(yhat,test_y))+0.13)
