...
def get_train_user_dict(base_path,target):
    ...
    #feature_list.extend(cur_folder_list)
    feature = pd.DataFrame(cur_folder_list,columns=['area_id','answer']+[k for k in range(108,216)])
    feature.to_csv('./mid_part/train_feature_%d_down.csv'%target,index=False)
